...
void loop() { 
  ...
  Serial.println(val);           //Check sensor value in serial port // when val is less than the pre-set value, increase the angle 
  if(val<40) {
    pos = pos +2;           
    ...
    delay(100);                  //As the angle expands, the LED becomes brighter 
    ...
    myservo.write(pos);          //write angle of the servo           
    delay(100);                  //As the angle shrinks, the LED light becomes darker           
    light = map(pos,0,90,0,255);           
    analogWrite(LED,light);      //write the brightness value        
  } 
}
